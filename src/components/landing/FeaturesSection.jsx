...
export default function FeaturesSection() {
  ...
  useGSAP(() => {
    ...
    // 1. Cover Reveal Slide-up animation (Scrubbed with scroll)
    gsap.fromTo(contentRef.current,
      { y: '100vh' },
      {
        ...
        scrollTrigger: {
          ...
          start: 'top bottom', // when wrapper top enters viewport bottom
          end: 'top top',       // when wrapper top reaches viewport top
          scrub: true,
        }
      }
    );

    // 2. Feature Cards Staggered Reveal Animation (One-time on enter)
  ...
  return (
    <div ref={wrapperRef} className="relative z-30">
      <section
        id="features"
        ref={contentRef}
        className="py-24 px-6 md:px-12 relative overflow-hidden bg-gradient-to-b from-transparent to-[var(--bg-tertiary)] bg-[var(--bg-primary)] rounded-t-[2.5rem] md:rounded-t-[3.5rem] shadow-[-2px_-15px_30px_rgba(0,0,0,0.06)] dark:shadow-[-2px_-15px_40px_rgba(0,0,0,0.5)] border-t border-[var(--border-color)]/30"
      >
